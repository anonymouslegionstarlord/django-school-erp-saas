...
def workspace_required(view_func):
    ...
    def wrapped(request, *args, **kwargs):
        ...
        if membership is None:
            raise PermissionDenied("Your account does not belong to a ReturnRelay workspace.")
...
def manager_required(view_func):
    ...
    def wrapped(request, *args, **kwargs):
        if not request.membership.can_manage:
            raise PermissionDenied("Claims manager access is required.")
        return view_func(request, *args, **kwargs)
...
def inspector_required(view_func):
    ...
    @wraps(view_func)
    def wrapped(request, *args, **kwargs):
        if not request.membership.can_inspect:
